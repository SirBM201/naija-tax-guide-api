...
def _build_telegram_return_link() -> str:
    bot_username = _clean(request.args.get("bot")) or _clean(
        __import__("os").getenv("TELEGRAM_BOT_USERNAME")
    )
    if bot_username:
  ...
  <div class="wrap">
    <div class="card">
      ...
      <div class="meta">
        ...
        <div><strong>Plan:</strong> {escape(plan_code or "Not available")}</div>
...
def channel_payment_return():
    ...
    try:
        ...
        return _render_channel_return_page(
            ...
            button_label="Return to Telegram" if channel_type == "telegram" else "Open channel",
            button_url=_build_telegram_return_link() if channel_type == "telegram" else _build_whatsapp_return_link(provider_user_id),
            reference=reference,
            plan_code=plan_code,
