...
def subscription_activate():
    ...
    Body:
      {
        "account_id": "<uuid>",
        "plan_code": "monthly|quarterly|yearly|manual",
...
def subscription_change():
    ...
    try:
        if when == "at_expiry":
            ...
            return jsonify({"ok": True, "mode": "scheduled", "subscription": sub}), 200
        else:
            sub = activate_subscription_now(account_id=account_id, plan_code=new_plan_code, status="active")
            return jsonify({"ok": True, "mode": "activated", "subscription": sub}), 200
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
